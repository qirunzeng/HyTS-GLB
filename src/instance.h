...
inline Instance generate_synthetic_instance(int K, int d, double S, RNG& rng) {
    ...
    if (d == 1) {
        ...
        for (int i = 0; i < K; ++i) {
            const double t = (double)i / (double)(K - 1);     // in [0,1]
            const double r = (1.0 - 2.0 * t) * r_max;         // +r_max ... -r_max
            const double a = r / theta_norm;                  // since dot(x,theta)=a*||theta||=r
            inst.x[i][0] = a;                                 // ||x|| = |a| <= 1
        }
        return inst;
    }
    ...
    for (int i = 0; i < K; ++i) {
        ...
        const double r = (1.0 - 2.0 * t) * r_max;             // +r_max ... -r_max
        double a = r / theta_norm;                            // ensures dot(a*u, theta)=r
        if (a >  1.0) a =  1.0;                               // safety clamp
        if (a < -1.0) a = -1.0;
        ...
        if (!ok) {
            // Very unlikely fallback: pick a deterministic orthogonal vector
            // Find an index where u has not-too-large magnitude and create v orthogonal to u in that plane
            int p = 0;
        ...
        Vec xi(d);
        for (int j = 0; j < d; ++j) xi[j] = a * u[j] + b * v[j];

        // Now dot(xi, theta) = dot(a u + b v, theta_norm u) = a * theta_norm = r (exact),
        // and ||xi|| = 1 (unless numerical rounding), so xi is feasible (in unit ball).
        inst.x[i] = xi;
    }
